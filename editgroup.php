            ...
            <div class="form-group">
                ...
                <div class="col-md-8">
                    <select for="add-user" name="add-user[]" class="chosen-select" data-placeholder="Click here.." style="width: 250px;" multiple>
                        <option></option>
                        <?php
                        $sql = "SELECT id, email FROM `users` WHERE users.id NOT IN (SELECT unique_id FROM users_groups WHERE users_groups.group_id = '$logid' )";
                    ...
                    <div class="panel">
                        <div class="panel-body">
                            <table class="table table-striped table-bordered table-hover" id="dataTable2">
                                ...
                                <tbody>
                            <?php
                            $sql = "SELECT users.email, users.id, users_groups.group_id FROM `users` INNER JOIN `users_groups` ON users.id=users_groups.unique_id WHERE users_groups.group_id = '$logid' ORDER BY users.email ASC";
                            $result = $db->prepare($sql);
                            $result->execute();
                            $stop = $adminfunctions->createStop($session->email, 'delete-groupmembership');
                            while ($row = $result->fetch()) {
                                echo "<tr><td>" . $row['email'] . "</td>";
                                echo "<td class='text-center'><div class='btn-group btn-group-xs'>";
                                if ($row['email'] != ADMIN_NAME) {
