...
<div class="sidebar-right">
    <div id="right-sidebar-id">
        ...
        <div class="right-sidebar-section">
            <?php
            $sql5 = "SELECT * FROM users WHERE email != '" . ADMIN_NAME . "' ORDER BY timestamp DESC LIMIT 1";
            $result5 = $db->prepare($sql5);
            $result5->execute();
            $row5 = $result5->fetch();
            $lastlogin = $adminfunctions->displayDate($row5['timestamp']);
            echo $row5['email']." logged on - ".$lastlogin;
            ?>
        </div>        
        ...
        <div class="right-sidebar-section">
            <?php
            $adminactivation = $adminfunctions->displayAdminActivation('created_at');
            ...
            echo "<p><i class='oi oi-circle-check'></i> ".$adminfunctions->usersSince($session->email) . " new users have registered since your last visit.</p>";
